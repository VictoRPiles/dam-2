...
class inventario(models.Model):
    ...
    tipo_producto = fields.Selection([('Abeto', 'Abeto'), ('Pino', 'Pino'), ('Roble', 'Roble')],
    ...
    def _calcular_dinero_gastado(self):
        ...
        if compras:
            for compra in compras:
                nombre_producto = compra.inventory_id.name
                precio_producto = compra.inventory_id.price_per_unit
                cantidad_copmpra = compra.quantity
                coste_compra = precio_producto * cantidad_copmpra

                print("\tCompró " + str(cantidad_copmpra) + " unidades de " + str(nombre_producto) + " por " + str(
                    precio_producto) + " cada unidad, un total de: " + str(coste_compra))
...
class compra(models.Model):
    ...
    id_inventario = fields.Many2one(comodel_name = 'aag__cesta_compra.inventario', string = 'Inventario',
                                    required = True)
    ...
    def _check_stock_is_enough(self):
        ...
        stock = self.id_inventario.quantity
    ...
    def name_get(self):
        ...
        result = []
        for compra in self:
            name = compra.inventory_id.name + " " + compra.client_id.name
            print("\t" + name)
    ...
    def _descontar_cantidad_inventario(self):
        ...
        cantidadInventarioSinDescontar = self.id_inventario.quantity
        ...
        self.id_inventario.quantity = cantidadInventarioSinDescontar - cantidadCompra
